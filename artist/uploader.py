"""
Main upload orchestration logic for artists
"""
...
logger = logging.getLogger('artist_uploader.uploader')

class ArtistUploader:
    """Main uploader class that orchestrates the artist upload process"""
    ...
    def upload_artists(self) -> UploadStats:
        """
        Main method to upload artists
        ...
        try:
            ...
            for row_num, artist in self.csv_parser.parse_csv():
                if artist is None:
                ...
                if not artist.is_valid():
                    logger.warning(f"Row {row_num}: Invalid artist data - {artist.slug}")
                    with self.stats_lock:
                        self.stats.skipped += 1
                    continue
                
                # Log which fields will be updated
                fields_to_update = artist.get_fields_to_update()
                logger.debug(f"Row {row_num}: Will update fields {fields_to_update} for {artist.slug}")
                
                current_batch.append((row_num, artist))
    ...
    def _process_batch(self, batch_data: List[Tuple[int, ArtistData]]):
        ...
        with ThreadPoolExecutor(max_workers=self.config.processing.max_workers) as executor:
            # Submit all tasks
            future_to_data = {
                executor.submit(self.db_manager.upload_document, artist): (row_num, artist)
                for row_num, artist in batch_data
            }
            ...
            for future in as_completed(future_to_data):
                row_num, artist = future_to_data[future]
                ...
                try:
                    ...
                    logger.error(f"Unexpected error processing '{artist.slug}' (row {row_num}): {str(e)}")
                    with self.stats_lock:
                        self.stats.errors += 1
    ...
    def _log_final_stats(self):
        ...
        logger.info("\n" + "="*60)
        logger.info("ARTIST UPLOAD COMPLETED")
        logger.info("="*60)
